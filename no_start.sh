...
exec f5-rest-node /config/f5-cloud-libs/scripts/azure/runScripts.js --log-level debug --tag v1.0.0 --onboard --output /var/log/onboard.log --log-level debug --host ${myip} -u admin -p ${adminpass}} --hostname ${hostname}.${location}.cloudapp.azure.com --set-password admin:${adminpass} --db tmm.maxremoteloglength:2048 --module ltm:nominal --signal ONBOARD_DONE --network --wait-for ONBOARD_DONE --output /var/log/network.log --host ${hostname} -u admin -p ${adminpass} --multi-nic --default-gw ${mydg} --vlan vlan_mgmt,1.0 --self-ip self_mgmt,${myip},vlan_mgmt --log-level debug --background --force-reboot --signal NET_DONE
